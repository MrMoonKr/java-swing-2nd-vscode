...
public class FixedGlassPane extends JPanel implements MouseListener, MouseMotionListener, FocusListener {
     ...
     * drags that started with mouse presses over the check box. Otherwise, when the
     * user presses the check box then drags off, the check box isn't disarmed -- it
     * keeps its dark gray background or whatever its L&F uses to indicate that the
    ...
    private void redispatchMouseEvent( MouseEvent e ) {
        ...
        if ( inMenuBar || inButton || inDrag ) {
            ...
            component.dispatchEvent( new MouseEvent( component, eventID, e.getWhen(), e.getModifiersEx(),
